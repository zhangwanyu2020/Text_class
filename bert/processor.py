...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  label_id = label_to_idlist(example.label,label_map)
  if ex_index < 5:
...
def label_to_idlist(label_text,label_map):
    label_seg = label_text.strip().split(' ')
    label_list = [0]*95
    for key in label_map.keys():
        for j in label_seg:
            label_list[label_map[j]] = 1
    return label_list
